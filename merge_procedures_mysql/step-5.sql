-- Step 4: Full procedure = Step 3 + log_v_i (insert view).
-- Mimics BigQuery merge.sql: create temp table log_v_i = latest non-delete per PK in CDC window,
-- then show its result (typed cols + source_ts_ns_order, publish_time, mysql_load_ts, pos).

-- saare values extract hone lag gai hai yaha pe abhi tak
...
proc_main: BEGIN
    ...
    -- ------------------------------------------------------------------
    -- 4) Build helper strings: view_cols, partition_by_str, join_on_str,
    --    delete_join_on_str, merge_on_str
    -- ------------------------------------------------------------------
    SET v_target_schema     = SUBSTRING_INDEX(v_target_fp, '.', 1);
    SET v_target_table_name = SUBSTRING_INDEX(v_target_fp, '.', -1);
    SET v_pk_first_col      = TRIM(SUBSTRING_INDEX(v_pk, ',', 1));
    ...
    SET v_partition_by_str = CONCAT(
        'JSON_UNQUOTE(JSON_EXTRACT(data, ''$.\"', v_pk_first_col, '\"''))'
    );

    SET v_join_on_str = CONCAT(
        'JSON_UNQUOTE(JSON_EXTRACT(d.data, ''$.\"', v_pk_first_col, '\"'')) = i.', v_pk_first_col
    );

    SET v_delete_join_on_str = CONCAT(
        'target.', v_pk_first_col,
        ' = JSON_UNQUOTE(JSON_EXTRACT(source.data, ''$.\"', v_pk_first_col, '\"''))'
    );

    SET v_merge_on_str = CONCAT(
        'target.', v_pk_first_col, ' = source.', v_pk_first_col
    );
    ...
    INSERT INTO _sql_parts (part_no, part_val) VALUES (
        3,
        CONCAT(
            ', l.source_ts_ns_order, l.pos, NOW(6) AS mysql_updated_on FROM ( ',
            '  WITH ranked AS ( ',
            '    SELECT d.*, ',
            '      ROW_NUMBER() OVER ( ',
            '        PARTITION BY JSON_UNQUOTE(JSON_EXTRACT(d.data, ''$.\"', REPLACE(v_pk_first_col, "'", "''"), '\"'')) ',
            '        ORDER BY ',
            '          CAST(REPLACE(REPLACE(JSON_UNQUOTE(JSON_EXTRACT(d.data, ''$.\"__ts_ns\"'')), ''T'', '' ''), ''Z'', '''')) AS DATETIME) DESC, ',
            ...
            '  ) ',
            '  SELECT data, ',
            '    CAST(REPLACE(REPLACE(JSON_UNQUOTE(JSON_EXTRACT(data, ''$.\"__ts_ns\"'')), ''T'', '' ''), ''Z'', '''')) AS DATETIME) AS source_ts_ns_order, ',
...
-- ------------------------------------------------------------------
    ...
    'WHERE d.rn = 1 ',
    '  AND ( ',
    '    i.', v_pk_first_col, ' IS NULL ',
    '    OR i.source_ts_ns_order < CAST(REPLACE(REPLACE(JSON_UNQUOTE(JSON_EXTRACT(d.data, ''$.\"__ts_ns\"'')), ''T'', '' ''), ''Z'', '''')) AS DATETIME) ',
    '  ) ',
    'ORDER BY ',
    '  CAST(REPLACE(REPLACE(JSON_UNQUOTE(JSON_EXTRACT(d.data, ''$.\"__ts_ns\"'')), ''T'', '' ''), ''Z'', '''')) AS DATETIME) DESC, ',
    '  CAST(JSON_UNQUOTE(JSON_EXTRACT(d.data, ''$.\"__source_pos\"'')) AS SIGNED) DESC'
