...
proc_main: BEGIN
    ...
    INSERT INTO _sql_parts (part_no, part_val) VALUES (
        3,
        CONCAT(
            ', l.source_ts_ns_order, l.pos, NOW(6) AS mysql_updated_on, l.__op FROM ( ',
            '  WITH ranked AS ( ',
            '    SELECT d.*, ',
            '      ROW_NUMBER() OVER ( ',
...
-- ------------------------------------------------------------------
    ...
    SET @stmt_sql = CONCAT(
        ...
        'INNER JOIN log_v_i AS i ON target.', v_pk_first_col, ' = i.', v_pk_first_col, ' AND i.`__op` = ''u'' ',
    ...
    SET @stmt_sql = CONCAT(
        ...
        '  WHERE t.', v_pk_first_col, ' = i.', v_pk_first_col, ' ',
        ')'
    );
